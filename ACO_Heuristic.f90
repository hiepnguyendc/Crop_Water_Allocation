            ...
            if (seasons(cur_sea)%wuse_crop(r) >= 0) then
                if (avai_water >= array_areas(dpts)*(seasons(cur_sea)%wuse_crop(r)-1)*wcap) then
                    ...
                    tree(itr)%dec(dpts)%season(cur_sea)%opt_crop(r)%cost = &
                        ...
                        seasons(cur_sea)%cc(r,1)*seasons(cur_sea)%cc(r,2)*&
                        (cur_water**seasons(cur_sea)%cc(r,3)))*seasons(cur_sea)%pcrop(r)&
                        -(seasons(cur_sea)%pcost(r)+pwater*cur_water)
                else
                    tree(itr)%dec(dpts)%season(cur_sea)%opt_crop(r)%cost = 0
                end if
            else
                cur_water = int((-seasons(cur_sea)%bb(r,1)*seasons(cur_sea)%bb(r,2)/&
                    (seasons(cur_sea)%cc(r,1)*seasons(cur_sea)%cc(r,2)*&
                    seasons(cur_sea)%cc(r,3)))**(1/(seasons(cur_sea)%cc(r,3)-1)))
