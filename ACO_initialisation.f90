...
subroutine initialise_ant_graph
	...
	do i=1,n_tree
        ...
        read(33,*) (array_areas(k),k=1,10,1)
        read(33,*) (array_areas(k),k=11,20,1)
        read(33,*) (array_areas(k),k=21,30,1)
        read(33,*) (array_areas(k),k=31,40,1)
        read(33,*) (array_areas(k),k=41,50,1)
        ...
        do j = 1, n_sea
            ...
            allocate(seasons(j)%dd(n_crop(j),3))
        ...
        do k = 1, n_sea
            ...
            do j = 1, n_crop(k)
                ...
                if (cropname /= "dryland") then
                    READ(223,*) seasons(k)%aa(j,1),seasons(k)%bb(j,1),seasons(k)%cc(j,1), seasons(k)%dd(j,1)
                    READ(223,*) seasons(k)%aa(j,2),seasons(k)%bb(j,2),seasons(k)%cc(j,2), seasons(k)%dd(j,2)
                    READ(223,*) seasons(k)%aa(j,3),seasons(k)%bb(j,3),seasons(k)%cc(j,3), seasons(k)%dd(j,3)
                end if
            end do
        end do
        READ(223,*)
			    ...
			    do l=1,n_crop(k)
                    ...
                    if (seasons(k)%name_crop(l) == "dryland") then
                        ...
                        tree(i)%dec(j)%season(k)%opt_crop(l)%cost = (seasons(k)%aa(l,1)*&
                            ...
                            seasons(k)%dd(l,1)*seasons(k)%dd(l,2)*(x(l)**seasons(k)%dd(l,3)))*&
                            seasons(k)%pcrop(l)-(seasons(k)%pcost(l)+pwater*x(l))

                        seasons(k)%n_opt_water(l) = 19
                        interval(l) = wcap
				        ...
				        do r=1,seasons(k)%n_opt_water(l)
                            ...
                            tvar = (seasons(k)%aa(l,1)*seasons(k)%aa(l,2) + &
                                seasons(k)%bb(l,1)*seasons(k)%bb(l,2)*y(r) + &
                                seasons(k)%cc(l,1)*seasons(k)%cc(l,2)*(y(r)**seasons(k)%cc(l,3))+&
                                seasons(k)%dd(l,1)*seasons(k)%dd(l,2)*(y(r)**seasons(k)%dd(l,3)))*&
                                seasons(k)%pcrop(l)-(seasons(k)%pcost(l)+pwater*y(r))
                            if (tvar > 0) then
