...
subroutine global_routines(num_it)
  ...
  integer :: num_it
    ...
    if(aco_type==1) call global_updating_AS
...
!:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
 ...
 subroutine global_updating_AS
	...
	integer :: i, j,k,selected1,selected2,selected3,p                      ! counters  
    REAL(8) :: tau_current_water, tau_new_water							! pheromone value1s
    REAL(8) :: tau_current_land, tau_new_land   						! pheromone value1s
	real(8) :: small_val = 0.00000000001
    integer :: sel_water,sel_land,sel_crop							! used to determine tau
	...
	do i = 1, n_tree
		do j = 1, max_ant                       ! iterating through colony of ants
			if(ant(j)%val < small_val)then      ! determining amount of pheromone to add
				tau_add = q / small_val
			else
				tau_add = q /ant(j)%val
            end if
		
			DO k = 1, max_path                  ! iterating through edges selected for ant i
                do p = 1, n_sea
                    sel_crop = ant(j)%tree(i)%season(p)%dec_crop(k)
				    tau_current_crop = tree(i)%dec(k)%season(p)%opt_crop(sel_crop)%tau
				    tau_new_crop = tau_current_crop + tau_add                           ! calculating new pheromone intensity
				    tree(i)%dec(k)%season(p)%opt_crop(sel_crop)%tau = tau_new_crop
                    
				    do r = 1, n_crop(p)
                        if (seasons(p)%name_crop(r) /= "dryland") then
					        sel_water = ant(j)%tree(i)%season(p)%crop(r)%dec_water(k)
					        tau_current_water = tree(i)%dec(k)%season(p)%crop(r)%opt_water(sel_water)%tau
					        tau_new_water = tau_current_water + tau_add                 ! calculating new pheromone intensity
					        tree(i)%dec(k)%season(p)%crop(r)%opt_water(sel_water)%tau = tau_new_water
                        end if
                    end do
                end do
			END DO
 ...
 end subroutine global_updating_AS
...
! determining global best path/solution
	...
	best_i%val = high_val                         ! setting to a high value1
	DO i = 1, max_ant
