...
! if (tree(itr)%dec(dpts)%season(cur_sea)%opt_crop(r)%prob == 0.0) ....
    ...
    tree(itr)%dec(dpts)%season(cur_sea)%crop(cur_crop)%opt_water(:)%prob = 0
                ...
                if (ant(n_ant)%tree(itr)%season(cur_sea)%crop(cur_crop)%dec_water_status(r) == 0) then
                    y = tree(itr)%dec(dpts)%season(cur_sea)%crop(cur_crop)%opt_water(r)%property
                        tvar = (seasons(cur_sea)%aa(cur_crop,1)*seasons(cur_sea)%aa(cur_crop,2) + &
                            ...
                            seasons(cur_sea)%cc(cur_crop,1)*seasons(cur_sea)%cc(cur_crop,2)*(y**seasons(cur_sea)%cc(cur_crop,3))+&
                            seasons(cur_sea)%dd(cur_crop,1)*seasons(cur_sea)%dd(cur_crop,2)*(y**seasons(cur_sea)%dd(cur_crop,3)))*&
                            seasons(cur_sea)%pcrop(cur_crop)-(seasons(cur_sea)%pcost(cur_crop)+pwater*y)
                        if (tvar > 0) then
